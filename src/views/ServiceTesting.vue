  ...
  <v-flex>
    <v-container fluid>
      <v-card>
          ...
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item>
                <v-list-item-content>
                  <file-upload
                    ...
                    v-model="forms"
                    @input="convertForm"
                  >Convert form to TypeScript</file-upload>
                </v-list-item-content>
              </v-list-item>
              <v-list-item @click="addAlert">
                ...
                <v-list-item-content>Add Alert</v-list-item-content>
              </v-list-item>
              <v-list-item @click="throwError">
                <v-list-item-action>
                  <v-icon>mdi-alert-circle</v-icon>
                </v-list-item-action>
                <v-list-item-content>Throw error</v-list-item-content>
              </v-list-item>
            </v-list>
          </v-menu>
        </v-toolbar>
        <v-list>
          <v-list-item v-for="name in testModules" :key="name">
            <v-checkbox v-model="modulesToRun" :value="name" :label="name" />
          </v-list-item>
        </v-list>
        <v-toolbar flat>
        ...
        <v-card-actions>
          <v-btn :disabled="state !== WAITING" @click="start()">
            <span v-if="state === WAITING">Start</span>
            <v-progress-circular v-else />
          </v-btn>
          <v-checkbox label="Debug errors" v-model="debugErrors" />
        </v-card-actions>
      </v-card>
    </v-container>
  </v-flex>
