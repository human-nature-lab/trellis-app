        ...
        checkDownloadSize: function () {
          ...
          SyncService.getSnapshotFileSize(this.source, this.snapshotId).then((snapshotFileSize) => {
            ...
            if (snapshotFileSize > freeDiskSpace) {
              this.warning = true
              this.warningMessage = `The snapshot is ${formatBytesFilter(snapshotFileSize)} and you only have ${formatBytesFilter(freeDiskSpace)} free.`
            } else if ((snapshotFileSize * 5) > freeDiskSpace) {
              this.warning = true
              this.warningMessage = `The extracted snapshot requires ~${formatBytesFilter(snapshotFileSize * 5)} and you only have ${formatBytesFilter(freeDiskSpace)} free.`
            } else {
              this.onDone()
            }
            this.checking = false
