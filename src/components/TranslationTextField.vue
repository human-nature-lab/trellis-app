      ...
      async onSave(newText) {
        ...
        if (this.persist) {
          ...
          for (let i = 0; i < this.memCopy.translationText.length; i++) {
            ...
            const tt = this.translation.translationText.find(
              (t) => t.localeId === mTt.localeId
            );
            if (!tt) {
              // Save a new translation text
              this.memCopy.translationText[
                i
              ] = await TranslationTextService.createTranslationText(
                this.translation.id,
                mTt
              );
      ...
      async loadLocales(): Promise<void> {
        ...
        try {
          ...
          if (!this.locales.length) {
            const locales = await LocaleService.getStudyLocales(
              this.global.study.id
            );
